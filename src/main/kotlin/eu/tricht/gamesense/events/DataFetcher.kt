...
class DataFetcher() {
    ...
    private val players = "(Spotify|MusicBee|AIMP|YouTube Music Desktop App|TIDAL|Qobuz|Cider).exe".toRegex()
        ...
        val callback = WinUser.WNDENUMPROC { hwnd, _ ->
            ...
            if (processPath.contains(players)) {
                ...
                if (wText.contains(" - ")) {
                    ...
                    if (processPath.endsWith("Cider.exe")) {
                        // Cider (Apple Music client) window title format handling
                        // Remove common Cider suffixes if present
                        song = song.replace(" - Cider", "")
                    }
                }
            }
